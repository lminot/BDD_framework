Feature: Open ticketmaster.com on a web browser and surf around 
	I am going to search for Garth Brooks tickets with 3 popular browsers

@smoke 
Scenario: Search for Garth Brooks Tickets in firefox 
	Given that I have loaded "www.ticketmaster.com" in a "firefox" 
	When I load a page 
	Then search for the term "Garth Brooks" 
	
@smoke 
Scenario: Search for Garth Brooks Tickets in chrome 
	Given that I have loaded "www.ticketmaster.com" in a "chrome" 
	When I load a page 
	Then search for the term "Garth Brooks" 
	
@smoke 
Scenario: Search for Garth Brooks Tickets in internet explorer 
	Given that I have loaded "www.ticketmaster.com" in a "internet explorer" 
	When I load a page 
	Then search for the term "Garth Brooks"

@longrunning
Scenario Outline: Search for Garth Brooks Tickets in chrome 
	Given that I have loaded "www.ticketmaster.com" in a "chrome" 
	When I load a page 
	Then search for the term "<artist>" 
	
	Examples: 
		|artist|
		|6 Shot, Baby, Lil Wayne & Miri Ben-Ari|
		|A Tribe Called Quest|
		|Algebra Blessett, Anthony Hamilton & Miri Ben-Ari|
		|Alpha Pup Records|
		|America's Next Top Model|
		|Amil, Ja Rule & JAY Z|
		|Anthony Hamilton, Miri Ben-Ari & Scarface|
		|AudibleTreats.com|
		|Beyoncé|
		|Biz Markie|
		|Blackalicious|
		|Boogie Down Productions|
		|Boss|
		|Busta Rhymes|
		|Cali Swag District|
		|Chamillionaire|
		|Chamillionaire feat. Slick Rick|
		|Colt Ford|
		|Consequence, John Legend & Miri Ben-Ari|
		|Coolio|
		|Craig Mack|
		|Crazy Frog|
		|D-Block|
		|DJ Jazzy Jeff featuring Twone Gabz|
		|DJ Khaled|
		|DJ Shadow|
		|DMX|
		|Dance Moms|
		|Das EFX|
		|Dead Prez|
		|Dead Prez featuring Tahir & People's Army|
		|Digital Underground|
		|Dirty Heads|
		|Doom & Ghostface Killah|
		|Dr. Dre|
		|Dr. Dre & Blackstreet|
		|Eminem|
		|En Vogue|
		|Eric B. & Rakim|
		|Erykah Badu|
		|Fabolous, Kanye West, Miri Ben-Ari & Musiq Soulchild|
		|Father MC|
		|Fatman Scoop, Lennox, Miri Ben-Ari, Pitbull & Zion|
		|Forever the Sickest Kids|
		|Fugees|
		|HIP HOP|
		|Handsome Boy Modeling School featuring Lord Finesse, Mike Shonoda, Chester Bennington, Rahzel, Qbert, Grand Wizard Theodore & Jazzy Jay / featuring Tim Meadows|
		|Heavy D & The Boyz|
		|Hip-Hop In Ya Ear|
		|Hopsin|
		|Ice-T|
		|Immortal Technique|
		|Indie Music - Comedy, Rock, Pop, Hip Hop, Dance, Electronic and Alt|
		|Instrumental Hip Hop Beat Makers|
		|JAY Z|
		|Jazz Addixx|
		|Joell Ortiz|
		|Jorge Quintero|
		|Junior M.A.F.I.A.|
		|KRS-One|
		|Knoc-Turn'al & Snoop Dogg|
		|Kyle Massey|
		|LL Cool J|
		|Left Lane Cruiser|
		|Lil Jon & The East Side Boyz & Ying Yang Twins|
		|Locksmith|
		|Love & Hip Hop: Hollywood|
		|Ludacris|
		|Lupe Fiasco|
		|Lupe Fiasco feat. Nikki Jean|
		|MC Hammer|
		|Marky Mark and the Funky Bunch|
		|Mary J. Blige & Method Man|
		|Mash-Up|
		|Master Hit|
		|Mike Birbiglia|
		|Mike Jones|
		|Miri Ben-Ari|
		|Miri Ben-Ari & Akon|
		|Miri Ben-Ari & Doug E. Fresh|
		|Miri Ben-Ari & Pharoahe Monch|
		|Miri Ben-Ari & Styles P|
		|Mos Def|
		|Motova8|
		|N.A.S.A.|
		|Nas|
		|Nas featuring Jay-Z|
		|Nas featuring Kanye West|
		|Nas featuring Snoop Dogg|
		|Nas featuring The Game and Marsha Ambrosius|
		|Nas featuring Will.I.Am|
		|Natasha Leggero|
		|Naughty By Nature|
		|Nice & Smooth|
		|Nio tha Gift|
		|Ol' Dirty Bastard|
		|Onyx|
		|Outkast|
		|P. Diddy|
		|P. Diddy, Black Rob & Mark Curry|
		|PaceWon & Mr.Green|
		|Paul Wall|
		|Pentatonix|
		|People Under the Stairs|
		|Pete Rock & C.L. Smooth|
		|Portugal. The Man|
		|Positive K|
		|Project Pat, Three 6 Mafia & La Chat|
		|Public Enemy|
		|Puff Daddy & Faith Evans|
		|Queen Latifah & Monie Love|
		|Rakim|
		|Ray J|
		|Rob Base & DJ EZ Rock|
		|Royce da 5'9\"|
		|Rucka Rucka Ali|
		|Run-DMC|
		|Salt-n-Pepa|
		|Sir Mix-A-Lot|
		|Snoop Dogg & Dat Nigga Daz|
		|Static-X & Dead Prez|
		|Step Up Revolution Dance Workout|
		|Studio Group|
		|T.I.|
		|Tabata Songs|
		|The Hit Nation|
		|The Notorious B.I.G.|
		|The Pharcyde|
		|The Sugarhill Gang|
		|Three 6 Mafia|
		|Tricky|
		|Trip Lee|
		|United Ladies of Hip-Hop & R&B|
		|Vanilla Ice|
		|Visioneers|
		|Warren G & Nate Dogg|
		|Wu-Tang Clan|
		|Wyclef Jean|
		|Xzibit|
		|Ying Yang Twins|
		|Young Mozart|
		|Zumba Fitness|
		|e-dubble|